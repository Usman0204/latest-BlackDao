      ...
      <Box className="dialog-container" display="flex" alignItems="center" justifyContent="space-between">
        ...
        <Grid component="label" container alignItems="center" spacing={1} wrap="nowrap">
          ...
          <Grid item>
            gBLKD
            <InfoTooltip
              message={`Toggle to switch between ${view === 0 ? "staking to" : "unstaking from"} sBLKD or gBLKD`}
              children={undefined}
            />
          </Grid>
        </Grid>
        ...
        {checked && Number(quantity) ? (
          <Typography variant="body2">
            {view === 0
              ? `Stake ${ohmQuantity} BLKD → ${gohmQuantity} gBLKD`
              : `Unstake ${gohmQuantity} gBLKD → ${ohmQuantity} BLKD`}
          </Typography>
        ) : null}
      </Box>
    </Paper>
