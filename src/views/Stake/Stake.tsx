...
const Stake: React.FC = () => {
  ...
  const onChangeStake = async (action: string) => {
    ...
    if (action === "stake" && gweiValue.gt(ethers.utils.parseUnits(ohmBalance, "gwei"))) {
      return dispatch(error(t`You cannot stake more than your BLKD balance.`));
    }
              ...
              {!address ? (
                <div className="stake-wallet-notification">
                  ...
                  <Typography variant="h6">
                    <Trans>Connect your wallet to stake BLKD</Trans>
                  </Typography>
                </div>
              ) : (
                <>
                    ...
                    <Accordion className="stake-accordion" square defaultExpanded>
                      ...
                      <AccordionDetails>
                        ...
                        {Number(gOhmOnTokemak) > 0.00009 && (
                          <DataRow
                            title={`${t`gBLKD (Tokemak)`}`}
                            balance={`${trim(Number(gOhmOnTokemak), 4)} gBLKD`}
                            indented
                            isLoading={isAppLoading}
                          />
                        )}
